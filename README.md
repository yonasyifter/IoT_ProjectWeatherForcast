<<<<<<< HEAD
# Smart Park — Docker & Routing Setup

## URL Structure

```
http://localhost/          → 302 redirect → /hike
http://localhost/hike      → User-side Vue app  (Giganti della Sila)
http://localhost/admin     → Admin-side Vue app (IoT Dashboard)
...
Browser
  │
  ▼
nginx (port 80)  ← docker-compose maps host :80 to this container
  │
  ├─ /hike*   ──► user-side container (nginx, port 80)
  │                └─ serves dist/ built with base: '/hike'
  │                   Vue Router (createWebHistory) handles /hike/*
  │
  ├─ /admin*  ──► admin-side container (nginx, port 80)
  │                └─ serves dist/ built with base: '/admin'
  │                   Admin uses manual page switching (no Vue Router)
  │
...
```
User visits /hike/login
  │
  ├─ Enters credentials → Firebase signInWithEmailAndPassword
  │
  ├─ Fetches Firestore users/{uid}.role
  │     │
  │     ├─ role === 'admin'  → window.location.href = '/admin'
  │     │                       (hard navigate loads admin Vue bundle)
  │     │
  │     └─ role === 'user'   → router.push('/hike/trail-preferences')
  │
  └─ New registrations → always get role: 'user' in Firestore
```
...
# 3. Visit
open http://localhost/hike    # User app
open http://localhost/admin   # Admin app
```
...
npm install
npm run dev
# Runs at http://localhost:5174/hike
...
├── nginx.conf                  ← root reverse proxy (routes /hike and /admin)
├── .env.example
├── web/
│   ├── user-side/
│   │   ├── Dockerfile          ← node:20 build → nginx:alpine serve
│   │   ├── nginx.conf          ← SPA fallback for /hike/*
│   │   ├── .dockerignore
│   │   ├── vite.config.js      ← base: '/hike', port: 5174
│   │   └── src/
│   │       ├── router/index.js ← createWebHistory(), routes at /hike/*
│   │       └── views/LoginView.vue ← Firebase auth + admin redirect
│   │
│   └── admin-side/
│       ├── Dockerfile          ← node:20 build → nginx:alpine serve
│       ├── nginx.conf          ← SPA fallback for /admin/*
│       ├── .dockerignore
│       ├── vite.config.js      ← base: '/admin', port: 5173
│       └── src/
│           ├── auth/adminAuthStore.js ← Pinia + Firebase, role check
│           └── views/AdminLoginView.vue
```

## Adding an Admin User

In Firebase console → Firestore → `users` collection, add a document with the user's UID:

```json
{
  "email": "admin@yourpark.it",
